...
> numbers **refreshed for pull `d9172de6` (2026-06-15)** — re-confirm with a quick grep for
> the function name if the file has changed since. (The S3/scan-manager cleanup `#913/#923`
> shifted lines; content is intact.)
...
> registered **unconditionally** (line 1197, *outside* any `#ifdef`). The *truly*
...
DUCKDB_CPP_EXTENSION_ENTRY(sirius, loader)     :1719
└─ LoadInternal(loader)                        :1668   the bootstrap — registers everything:
   ├─ GetCallbackManager().Register(…)         :1676   SiriusContextExtensionCallback (per-conn state)
   ├─ InitialGPUConfigs(config)                :1483   SET options (AddExtensionOption ×N)
   ├─ RegisterGPUFunctions(db)                 :1178   table functions (gpu_execution, pin_table, …)
   └─ OptimizerExtension::Register(…)          :1693   wire transparent hooks   · enables Step 1
                                                       (hook bodies in transparent/ — see that map)

explicit gpu_execution — Step 1b (per CALL gpu_execution('…')):
GPUExecutionBind()                             :513  ◀─ DuckDB binds the table function (once)
├─ ExtractPlan()                               :244   parse → optimize → resolve
│     └─ PrepareConnection()                   :206   disable incompatible optimizers (explicit-path analogue)
├─ SiriusGeneratePhysicalPlan()                :501   ─▶ plan generator (create_plan)
└─ build sirius_interface + wrap plan          :536/573   → SiriusTableFunctionData (bind state)
GPUExecutionFunction()                         :598  ◀─ DuckDB pulls rows (repeatedly)
├─ if plan_error ─▶ run_internal_cpu_fallback_query   :103   ─▶ DuckDB CPU
├─ iface->sirius_execute_query(…)              :615   ─▶ sirius_interface          · Step 3 → 9
│     └─ on error ─▶ run_internal_cpu_fallback_query  :103   ─▶ DuckDB CPU
└─ data.res->Fetch()                           :635   pull the next chunk
```
...
its `sirius_execute_query` at `:615` is the **same convergence point** as the transparent
...
|---|---|---|
| `DUCKDB_CPP_EXTENSION_ENTRY(sirius, loader)` | 1719 | C entry point; calls `LoadInternal`. |
| `void SiriusExtension::Load(ExtensionLoader& loader)` | 1702 | Thin wrapper over `LoadInternal`. |
| `static void LoadInternal(ExtensionLoader& loader)` | 1668 | The real bootstrap. **The `sirius_pre_optimizer_hook` / `sirius_optimizer_hook` setup just before `OptimizerExtension::Register` (1693) *is* the doc's Step 1**, but their bodies are in `src/transparent/`, not here. |
| `void SiriusExtension::InitialGPUConfigs(DBConfig& config)` | 1483 | Registers all `SET` options (the `Set*` callbacks at ~1283–1478). Not part of query flow. |
| `void SiriusExtension::RegisterGPUFunctions(DatabaseInstance& instance)` | 1178 | Registers the `gpu_execution` table function (and `pin_table`, profiler, etc.). |
...
|---|---|---|
| `unique_ptr<FunctionData> SiriusExtension::GPUExecutionBind(ClientContext&, TableFunctionBindInput&, vector<LogicalType>&, vector<string>&)` | 513 | **Step 1b.1** — re-parses the inner SQL, optimizes it, generates the Sirius physical plan. Also constructs the `sirius_interface` (line 536). |
| `unique_ptr<LogicalOperator> SiriusTableFunctionData::ExtractPlan(ClientContext&)` | 244 | The parse+optimize+resolve sub-part of 1b.1 (explicit-path equivalent of DuckDB's optimizer running). |
| `static unique_ptr<sirius::op::sirius_physical_operator> SiriusGeneratePhysicalPlan(ClientContext&, unique_ptr<LogicalOperator>&)` | 501 | The "generate the Sirius physical plan" part of 1b.1 — calls `create_plan()`, the **same "single source of truth"** the doc names in Step 1.3 (here at bind-time instead of in `OnFinalizePrepare`). |
| `void SiriusExtension::GPUExecutionFunction(ClientContext&, TableFunctionInput&, DataChunk&)` | 598 | **Step 1b.2** — calls `sirius_iface->sirius_execute_query(...)` at line 615. **That call is the handoff to Step 3** (`src/sirius_interface.cpp`); everything past it leaves this file. |
| `unique_ptr<QueryResult> run_internal_cpu_fallback_query(ClientContext&, Connection&, const string&, const string&)` | 103 | **Step 1b.3** — graceful CPU fallback on plan/execution error (also the file's tie-in to the doc's "Error Handling" section). |
...
206), called from `ExtractPlan`. Same idea, different trigger.
...
- **S3 rewrite target** (bind-time schema probe; loosely Step 6 scan, not in the
  main trace): `SiriusReadParquetBind` (134), `SiriusReadParquetFunction` (162),
  `SiriusReadParquetCardinality` (171).
- **`pin_table` caching feature** (its own subsystem): `PinTableBind` (794),
  `PinTableFunction` (844), `UnpinTableBind` (1072), `UnpinTableFunction` (1089).
- **Profiler / query-label helpers**: `ProfilerStartFunction` (1117),
  `ProfilerStopFunction` (1129), `SiriusSetQueryLabelBind/Function` (1146/1161).
- **Legacy `gpu_processing` path** (behind `#ifdef SIRIUS_ENABLE_LEGACY` — ignore
  per CLAUDE.md): `GPUProcessingBind` (388), `GPUProcessingFunction` (443),
  `GPUGeneratePhysicalPlan` (374), `GPUBufferInit*` (674/750).
...
is implemented here and ends at the `sirius_execute_query()` call on line 615. Both
doorways converge on `sirius_interface.cpp` (Step 3) — your next file. The bulk of
the ~1,720 lines (config setters, `pin_table`, registration boilerplate) is exactly
...
- **The bind → execute contract, as it shapes this whole file.** Almost every
  ...
  `GPUExecutionBind` (513) runs **once** — it parses/optimizes the inner SQL, calls
  `create_plan()`, and returns a `unique_ptr<FunctionData>`; then
  `GPUExecutionFunction` (598) is called **repeatedly** to stream result chunks,
  receiving that same state back. Once you see this rhythm, the file stops looking
  like ~1,720 scattered lines and resolves into ~10 of these pairs. Internalizing the
...
- **`SiriusTableFunctionData`** *(this file's `FunctionData` subclass; defined at line
  180)* — the concrete "state bundle" threaded from `GPUExecutionBind` to
  ...
  plan-error flags; its `ExtractPlan()` (244) and `PrepareConnection()` (206) methods
...
- **`ExtensionLoader`** — the handle DuckDB hands the extension at `LOAD`, used in
  `LoadInternal` (1668) to register functions and reach the `DatabaseInstance`
...
- **`sirius::sirius_interface`** *(usually `sirius_interface`; Sirius's own type)* —
  the GPU engine's front door. Constructed in `GPUExecutionBind` (536) and invoked in
  `GPUExecutionFunction` via `sirius_execute_query(...)` (615). That call is the exact
